...
describe('MinimalistConfetti (Magic UI bottom cannon)', () => {
  ...
  test('no se queda pegado en pantalla al disparar múltiples ráfagas consecutivas rápidas', async () => {
    const screen = await render(<MinimalistConfetti burstTrigger={1} />)
    expect(screen.queryByTestId('magic-confetti-overlay')).toBeTruthy()

    // Disparar una segunda ráfaga casi de inmediato (300ms después)
    await act(async () => {
      jest.advanceTimersByTime(300)
      screen.rerender(<MinimalistConfetti burstTrigger={2} />)
    })

    expect(screen.queryByTestId('magic-confetti-overlay')).toBeTruthy()

    // Avanzar el tiempo para que terminen ambas ráfagas
    await act(async () => {
      jest.advanceTimersByTime(3000)
    })

    // Ambas ráfagas deben haber desaparecido por completo
    expect(screen.queryByTestId('magic-confetti-overlay')).toBeNull()
  })
})
