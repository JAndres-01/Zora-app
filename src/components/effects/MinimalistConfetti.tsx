...
export const MinimalistConfetti = memo(function MinimalistConfetti({ burstTrigger }: ConfettiProps) {
  ...
  useEffect(() => {
    ...
    activeAnimationsRef.current.set(burstId, compositeAnim)

    const removeBurst = () => {
      activeAnimationsRef.current.delete(burstId)
      const timer = activeTimersRef.current.get(burstId)
      if (timer) {
        clearTimeout(timer)
        activeTimersRef.current.delete(burstId)
      }
    ...
    // Temporizador de seguridad garantizado para evitar cualquier bloqueo residual en pantalla
    const safetyTimer = setTimeout(() => {
      removeBurst()
    }, Math.max(1600, maxTotalDuration + 200))
    activeTimersRef.current.set(burstId, safetyTimer)
  }, [burstTrigger])
